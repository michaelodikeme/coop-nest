...
export default function AdminSavingsPage() {
  ...
  const [isViewWithdrawalModalOpen, setIsViewWithdrawalModalOpen] =
    useState(false);
  ...
  const {
    ...
    isLoading,
    error,
  } = useAdminSavings(currentPage, pageSize, filters);
  const { data: monthlySavings, isLoading: isLoadingMonthly } =
    useAdminMonthlySavings(
      filterYear,
      filterMonth,
      monthlySavingsPage,
  ...
  const { data: withdrawalStats, isLoading: isLoadingStats } =
    useWithdrawalStatistics();
  const {
    data: memberSummary,
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          ...
          >
            <Button
              ...
              variant="outlined"
              onClick={() =>
                router.push(`/admin/members/${row.original.id}/financial`)
              }
            >
              Details
            </Button>
  ...
  const monthlySavingsColumns: DataTableColumn<any>[] = [
    ...
    },
    {
      ...
      label: "Shares",
      accessor: (row) =>
        row.shares && row.shares[0] ? row.shares[0].totalValue || 0 : 0,
      Cell: ({ row }) => {
        let shareValue = 0;
    ...
    },
    {
      ...
      accessor: (row) => {
        ...
        const sharesAmount =
          row.shares && row.shares[0]
            ? Number(row.shares[0].totalValue || 0)
            : 0;
      ...
      Cell: ({ row }) => {
        ...
        const sharesAmount =
          row.original.shares && row.original.shares[0]
            ? Number(row.original.shares[0].totalValue || 0)
            : 0;
    ...
    },
    {
      ...
      Cell: ({ value }) => (
        <Chip
          label={value || "PENDING"}
          color={
            value === "COMPLETED" || value === "ACTIVE" ? "success" : "warning"
          }
          size="small"
          sx={{
            fontSize: "0.75rem",
  ...
  const withdrawalsColumns: DataTableColumn<any>[] = [
    ...
    },
    {
      ...
      Cell: ({ row }) =>
        row.original.amount ||
        formatCurrency(Number(row.original.rawAmount) || 0),
      filterable: false,
    },
    {
      id: "requestDate",
  ...
  const handleWithdrawalAction = async (
    action: "APPROVED" | "REJECTED" | "PROCESSING"
  ) => {
    ...
    savingsData.forEach((record) => {
      ...
      if (!memberMap.has(memberId)) {
        ...
        memberMap.set(memberId, {
          ...
          totalSharesAmount: record.shares?.[0]?.totalSharesAmount
            ? Number(record.shares[0].totalSharesAmount)
            : 0,
        ...
        if (
          new Date(record.lastDeposit) > new Date(existingRecord.lastDeposit)
        ) {
          existingRecord.totalSavingsAmount = Number(
            record.totalSavingsAmount || 0
          );
          existingRecord.totalSharesAmount = record.shares?.[0]
            ?.totalSharesAmount
            ? Number(record.shares[0].totalSharesAmount)
            : 0;
          existingRecord.totalGrossAmount = Number(
      ...
      {activeTab === 1 && (
        <Paper sx={{ p: 2 }}>
          ...
          {isLoadingMonthly ? (
            ...
            <Alert severity="info">
              No monthly savings records available for{" "}
              {new Date(2000, filterMonth - 1, 1).toLocaleString("default", {
                month: "long",
              })}{" "}
              {filterYear}
            </Alert>
            ...
            >
              ...
              <Box sx={{ display: "flex", gap: 2 }}>
                <FormControl size="small" sx={{ minWidth: 150 }}>
                  <InputLabel id="withdrawal-status-filter-label">
                    Status
                  </InputLabel>
                  <Select
        ...
        >
          <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
            <Typography variant="body2" color="text.secondary" paragraph>
              Upload an Excel file containing savings records. The file should
              include member IDs, amounts, and transaction dates.
            </Typography>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">
                  {formatCurrency(
                    Number(
                      selectedRecord.principalAmount ||
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">
                  {selectedRecord.reference || "N/A"}
                </p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-gray-500">ERP ID</h4>
                <p className="mt-1 text-sm text-gray-900">
                  {selectedRecord.erpId ||
                    selectedRecord.member?.erpId ||
                    "N/A"}
                </p>
              </div>
            </div>
            ...
            >
              <Typography variant="body2" color="text.secondary">
                ...
                <span className="font-mono">
                  {selectedRecord.id?.substring(0, 8) || "N/A"}
                </span>
              </Typography>
      ...
      >
        {selectedWithdrawal && (
          <div className="space-y-4">
            ...
            {selectedWithdrawal.approvalSteps &&
              selectedWithdrawal.approvalSteps.length > 0 && (
                <>
                  <Typography variant="subtitle1" sx={{ mt: 3 }}>
                    Approval Workflow
                  </Typography>
                  <Box sx={{ ml: 1 }}>
                    {selectedWithdrawal.approvalSteps.map(
                      (step: any, idx: number) => (
                        <Box
                          key={idx}
                          sx={{
            ...
            >
              ...
              selectedWithdrawal.status === "REVIEWED" ? (
                ...
                >
                  ...
                  >
                    This request is in {selectedWithdrawal.status.toLowerCase()}{" "}
                    status and cannot be modified.
                  </Alert>
                </Box>
              )}
            </PermissionGate>
