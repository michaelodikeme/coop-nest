...
export function PlanDetails({ id }: PlanDetailsProps) {
  ...
  const { data: balanceHistory, isLoading: isBalanceHistoryLoading, error: balanceHistoryError } = useBalanceHistory(id, {});
  ...
  const isActive = plan.status === PersonalSavingsStatus.ACTIVE && !hasPendingWithdrawal;
  ...
  return (
    <Box>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          <Card>
            ...
            <CardContent>
              <Typography variant="h4" fontWeight="bold" color={statusDisplay.color}>
                {statusDisplay.label}
              </Typography>
            </CardContent>
          </Card>
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          <Card>
            ...
            <CardContent>
              <Typography variant="h4" fontWeight="bold">
                {plan.targetAmount ? formatCurrency(plan.targetAmount) : 'N/A'}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              ...
              <Box>
                ...
                <Typography variant="body1">
                  {plan.pendingWithdrawal?.requestedAt ? new Date(plan.pendingWithdrawal.requestedAt).toLocaleDateString() : 'N/A'}
                </Typography>
              </Box>
            </Grid>
            
            <Grid size={{ xs: 12, md: 6 }}>
              {plan.pendingWithdrawal && plan.pendingWithdrawal.approvalSteps && (
                <Box>
          ...
          {tabValue === 1 && (
            <Box>
                      ...
                      <Grid size={{ xs: 12, md: 4 }}>
                        <Box sx={{ textAlign: 'center', p: 2 }}>
                          ...
                          {plan.targetAmount ? (
                            <>
                              <Typography variant="h4" fontWeight="bold">
                                {Math.min(100, Math.round((plan.currentBalance || 0) / plan.targetAmount * 100))}%
                              </Typography>
                              <LinearProgress 
                                variant="determinate" 
                                value={Math.min(100, ((plan.currentBalance || 0) / plan.targetAmount * 100))}
              ...
              {isBalanceHistoryLoading ? (
                ...
                <Alert severity="error">
                  Failed to load balance history data. Please try again later.
                </Alert>
              ) : (
            ...
            <Card>
              ...
              <CardContent>
                <Grid container spacing={2}>
                  ...
                  <Grid size={{ xs: 8 }}>
                    <Typography variant="body1">{plan.member.name}</Typography>
                  </Grid>
                  ...
                  <Grid size={{ xs: 8 }}>
                    <Typography variant="body1">{plan.member.department}</Typography>
                  </Grid>
                  ...
                  <Grid size={{ xs: 8 }}>
                    <Typography variant="body1">{new Date(plan.createdAt).toLocaleDateString()}</Typography>
                  </Grid>
                  ...
                  <Grid size={{ xs: 8 }}>
                    <Typography variant="body1">{new Date(plan.updatedAt).toLocaleDateString()}</Typography>
                  </Grid>
                  ...
                  {plan.planType && (
                    <>
                      ...
                      <Grid size={{ xs: 8 }}>
                        <Typography variant="body1">{plan.planType.name}</Typography>
                      </Grid>
                      ...
                      {plan.planType.description && (
                        <>
                          ...
                          <Grid size={{ xs: 8 }}>
                            <Typography variant="body1">{plan.planType.description}</Typography>
                          </Grid>
                        </>
                      )}
                    </>
      ...
      {/* Withdrawal Dialog */}
      <Dialog open={isWithdrawalOpen} onClose={() => setIsWithdrawalOpen(false)}>
