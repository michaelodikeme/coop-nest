...
const RecentTransactionsSidebar: FC<RecentTransactionsSidebarProps> = ({ limit = 20 }) => {
  ...
  const transactionItems = useMemo(() => {
    ...
    if (typeof transactionsResponse.data === 'object' && Array.isArray((transactionsResponse.data as any)?.data)) {
      return (transactionsResponse.data as any)?.data || [];
    }
    return Array.isArray(transactionsResponse.data) ? transactionsResponse.data : [];
  }, [transactionsResponse]);

  // RESTORED: Your original icon logic including string pattern matching
  const getTransactionIcon = (type: string, baseType: string, module: string) => {
    switch (type) {
      case 'SAVINGS_DEPOSIT': return <Savings sx={{ color: theme.palette.success.main }} />;
      case 'SAVINGS_WITHDRAWAL': return <AccountBalanceWallet sx={{ color: theme.palette.error.main }} />;
      case 'SHARES_PURCHASE': return <Paid sx={{ color: theme.palette.info.main }} />;
      case 'SHARES_REDEMPTION': return <AttachMoney sx={{ color: theme.palette.warning.main }} />;
      case 'LOAN_DISBURSEMENT': return <LocalAtm sx={{ color: theme.palette.success.main }} />;
      case 'LOAN_REPAYMENT': return <CreditCard sx={{ color: theme.palette.primary.main }} />;
    }

    if (type.includes('DEPOSIT')) return <Savings sx={{ color: theme.palette.success.main }} />;
    if (type.includes('WITHDRAWAL')) return <AccountBalanceWallet sx={{ color: theme.palette.error.main }} />;
    if (type.includes('PURCHASE')) return <Paid sx={{ color: theme.palette.info.main }} />;
    if (type.includes('REDEMPTION')) return <AttachMoney sx={{ color: theme.palette.warning.main }} />;
    if (type.includes('DISBURSEMENT')) return <LocalAtm sx={{ color: theme.palette.success.main }} />;
    if (type.includes('REPAYMENT')) return <CreditCard sx={{ color: theme.palette.primary.main }} />;
    
    switch (module) {
      case 'SAVINGS':
        return baseType === 'CREDIT' ? <Savings sx={{ color: theme.palette.success.main }} /> : <AccountBalanceWallet sx={{ color: theme.palette.error.main }} />;
      case 'LOAN':
        return baseType === 'CREDIT' ? <LocalAtm sx={{ color: theme.palette.success.main }} /> : <CreditCard sx={{ color: theme.palette.primary.main }} />;
      case 'SHARES':
        return baseType === 'CREDIT' ? <Paid sx={{ color: theme.palette.info.main }} /> : <AttachMoney sx={{ color: theme.palette.warning.main }} />;
      default:
        return baseType === 'CREDIT' ? <Add sx={{ color: theme.palette.success.main }} /> : <Remove sx={{ color: theme.palette.error.main }} />;
    }
  };
  ...
  const getModuleColor = (module: string) => {
    switch (module.toUpperCase()) {
      case 'SAVINGS': return theme.palette.primary.main;
      case 'LOAN': return theme.palette.secondary.main;
      case 'SHARES': return theme.palette.info.main;
            ...
            {transactionItems.map((transaction, index) => (
              <Box key={transaction.id || index}>
                <ListItem 
                  sx={{ 
                    px: 0, py: 1.5,
                    '&:hover': { backgroundColor: theme.palette.action.hover, cursor: 'pointer', borderRadius: 1 }
                  }}
                  onClick={() => console.log('Transaction details:', transaction)}
                >
                  <ListItemIcon sx={{ minWidth: 40 }}>
                    {getTransactionIcon(transaction.transactionType, transaction.baseType, transaction.module)}
                  </ListItemIcon>
                      ...
                      <Grid container spacing={1} alignItems="center">
                        ...
                        <Grid size={{ xs: 6 }} sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
                          <Chip
                            ...
                            color={transaction.baseType === 'CREDIT' ? 'success' : 'error'}
                            variant="outlined"
                            sx={{ height: 16, fontSize: '0.6rem', fontWeight: 600, mr: 1, width: '55px', '& .MuiChip-label': { px: 0.8 } }}
                          />
                        ...
                        <Grid container spacing={1} alignItems="center" sx={{ mt: 0.5 }}>
                          <Grid size={{ xs: 7 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                              ...
                              <Box sx={{ width: 8, height: 8, backgroundColor: getModuleColor(transaction.module), borderRadius: '50%', mx: 0.8 }} />
                              <Typography variant="caption" sx={{ color: getModuleColor(transaction.module) }}>
                                {transaction.module}
                              </Typography>
                            </Box>
                          </Grid>
