...
export class WithdrawalController {
  ...
  public async createWithdrawalRequest(
    ...
    try {
      ...
      if (req.user.biodataId !== validatedData.biodataId && !req.user.isAdmin) {
        throw new ApiError(
          "You can only request withdrawals from your own account",
          403
        );
      }
  ...
  //     try {
  //         console.log('Fetching withdrawal requests with query:', req.query);
  //         console.log('Fetching withdrawal request:', req);
  ...
  //         }
  //         console.log('Final parameters for fetching withdrawal requests:', params);
  ...
  public async getWithdrawalRequests(
    ...
    try {
      ...
      if (isAdmin) {
        ...
        // Only apply biodataId filter if explicitly provided in query params
        if (query.biodataId) {
          params.biodataId = query.biodataId;
        }
        // For SUPER_ADMIN with empty biodataId, we don't force any biodataId filter
      } else {
        // Non-admin users can only see their own requests
        if (req.user.biodataId) {
          params.biodataId = req.user.biodataId;
        } else {
          // Regular user without biodataId should see no requests
      ...
      console.log("Final parameters for fetching withdrawal requests:", params);
      const result = await SavingsWithdrawalService.getWithdrawalRequests(
        params
      );
  ...
  public async getWithdrawalRequestById(
    ...
    try {
      ...
      const result = await SavingsWithdrawalService.getWithdrawalRequestById(
        id
      );
   ...
   */
  public async getWithdrawalStatistics(
    req: AuthenticatedRequest,
    res: Response,
    next: NextFunction
  ) {
    try {
      ...
      const result = await SavingsWithdrawalService.getWithdrawalStatistics(
        filters
      );
