...
export class PersonalSavingsService {
    ...
    async getAllPersonalSavings(queryParams: IPersonalSavingsQueryParams, userId: string, isAdmin: boolean) {
        ...
        const formattedPendingRequests = pendingCreationRequests.map(request => {
        ...
        return {
            ...
            meta: {
                ...
                activeCount: plans.filter(p => p.status === PersonalSavingsStatus.ACTIVE).length,
                closedCount: plans.filter(p => p.status === PersonalSavingsStatus.CLOSED).length,
            }
        };        
    } catch (error: unknown) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
